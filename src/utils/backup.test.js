...
describe('buildBackupPayload', () => {
  test('wraps app data with app name, version, and a timestamp', () => {
    const payload = buildBackupPayload({ habits: [{ id: 1 }] });
  ...
  test('defaults every missing collection to an empty array', () => {
    const payload = buildBackupPayload({ habits: [] });
  ...
  test('carries through provided settings fields as-is', () => {
    const payload = buildBackupPayload({ habits: [], accent: 'orange', mode: 'dark', language: 'ar' });
...
describe('exportBackupToFile', () => {
  ...
  test('opens the share sheet when sharing is available', async () => {
    ...
    await exportBackupToFile(buildBackupPayload({ habits: [] }));
    expect(Sharing.shareAsync).toHaveBeenCalledTimes(1);
  });

  test('skips the share sheet when sharing is unavailable, but still writes the file', async () => {
    Sharing.isAvailableAsync.mockResolvedValue(false);
    const uri = await exportBackupToFile(buildBackupPayload({ habits: [] }));
...
describe('importBackupFromFile', () => {
  ...
  test('parses a valid backup file and returns its data object', async () => {

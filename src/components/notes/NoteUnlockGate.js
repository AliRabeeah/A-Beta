...
/**
 * Full-screen gate rendered by AddEditNoteScreen in place of the note editor
 * whenever the note being opened has isLocked === true and hasn't been
 * unlocked yet in this screen session. This is the ONE enforcement point
 * for per-note locking, regardless of how the note was reached (list tap,
 * search result, deep link, etc) — the editor never renders note content
 * until onUnlock() fires.
 */
export default function NoteUnlockGate({ onUnlock, onCancel }) {
  ...
  usePreventScreenCapture('note-unlock-gate');
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top + 30 }]}>
      ...
      <View style={styles.center}>
        ...
        <Text style={[styles.title, { color: colors.text }]}>{t('noteLockedTitle')}</Text>
        <Text style={[styles.subtitle, { color: colors.textSecondary }]}>{t('noteLockedSubtitle')}</Text>

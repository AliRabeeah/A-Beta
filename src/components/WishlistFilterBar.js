...
/**
 * Same collapsed-by-default pattern as Favorites' FilterBar, but for
 * wishlist tags: any number of tags can be active at once (an item shows
 * if it carries ANY of the selected tags), since tags here are a custom,
 * open-ended set rather than one fixed category per item.
 */
export default function WishlistFilterBar({ tags, counts, activeTagIds, onToggleTag }) {
  ...
  return (
    <View style={[styles.wrap, isRTL && { flexDirection: 'row-reverse' }]}>
      ...
      >
        <Chip
          active={activeTagIds.length === 0}
          expanded={expanded}
          color={colors.primary}
          colors={colors}
          onPress={() => onToggleTag(null)}
          label={t('filterAll')}
          count={total}
          icon="✦"
        />
        {tags.map((tag) => (
          <Chip
            ...
            label={tag.builtIn ? defaultWishlistTagLabel(tag, t) : tag.label}
